...
class Test_geonode_connection(unittest.TestCase):
    ...
    def test_specified_raster_resolution(self):
        ...
        # Download data again
        bbox = get_bounding_box_string(hazard_filename)
        H = download(INTERNAL_SERVER_URL, hazard_name, bbox, resolution=0.01)
        A = H.get_data()

        # Compare shapes - FIXME (Ole): Have to establish expected numbers here!
        #                               Not exactly sure what they should be....
        #                               something like (max-min)/res, ....
